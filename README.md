...
If using a Linux OS on a Virtual Machine, it seems that any shared folders to the machine hosting the VM, are not properly recognized by the KVM/QEMU that is generated by `kiwi-ng`'s boxbuild approach. This can lead to `chroot` errors and terminated installer generation. The recommendation in this case to clone the installer git directly into the VM directory and edit the relevant files (e.g. `rockstor.kiwi`) before executing the box build (or edit them outside of the VM and transfer them back into the VM's installer directory using a shared folder. At the end of installer creation the `.iso` file has to be then copied from the VM directory back onto the VM's host for further use.
